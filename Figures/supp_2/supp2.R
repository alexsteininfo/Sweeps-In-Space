...
setwd("/Users/stein02/Desktop/upload/Figures/supp_2/")
...
probs <- read_csv("data/sweeps-prob.csv")

param_wt <- read_csv("data/params_wt.csv")
meas_wt <- read_csv("data/wt_speeds.csv")
...
ggplot() + geom_histogram(aes(x=df_f$pop_radius_at_first_mut_init, 
                              y=..density..,linetype=legend_1), fill=cols[3], alpha=0.5, bins = 18) +
  geom_line(aes(x=xaxis, y=f_X, linetype=legend_2), col=cols[3], linewidth = 1.5) +
...
fig <- ggplot() + geom_histogram(aes(x=df_f$pop_radius_at_first_mut_init, y=..density..,linetype=legend_1),
                                   fill=cols[4], alpha=0.25, bins=18) +
...
a = 1.5
text <- paste("/Users/stein02/Desktop/Plots for the Sweeps/supp_2/figures/","mu", mu, "s", s_m, ".png",sep = "")
