...
data <- read_csv("data/df2_unlimitedv2.csv")
...
a = 1.5
text <- paste("/Users/alexanderstein/Documents/Sweeps-project/figures/randomU/stoppingcriterium", ".png",sep = "")
...
data_sweepsonly <- data_sweepsonly %>% mutate(radius_end = sqrt(num_cell/pi))
...
#ggsave("combined_plot.png", grid.arrange(plot1, plot2, plot3, ncol = 3, widths = c(2,1,1)), width = 12, height = 4)

ggsave("figures/randomU/AssessGammaPred_unlimited.png", plot1, width = 5*4/3, height = 5)
...
a = 1.5
text <- paste("/Users/alexanderstein/Documents/Sweeps-project/figures/randomU/gammafitsUnlimited", ".png",sep = "")
ggsave(text, fig_fits, width = 4^a, height = 3^a)

for (s_select in s_driver_unique) {
  fig_densities <- ggplot(filter(data_sweepsonly, s_driver_birth == s_select), aes(x = radius_end)) +
    geom_histogram(aes(y = after_stat(density)), bins = 15, fill = "black", color = "black") +
  ...
  a = 1.5
  text <- paste("/Users/alexanderstein/Documents/Sweeps-project/figures/randomU/radiusatsweepcompletion", "s", s_select, ".png",sep = "")
  ggsave(text, fig_densities, width = 4^a, height = 3^a)
}
